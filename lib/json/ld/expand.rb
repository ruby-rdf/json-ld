...
module JSON::LD
  ...
  module Expand
    ...
    # @param [Hash] content (nil)
    #   Content to merge into a node definition
    ...
    def expand(input, active_property, context, ordered: true)
      ...
      when Array
        ...
        value
      when Hash
        # If active_property has an @content definition, which has an @context, merge it after any @context in element.
        # Note, content that contains a sub-@context is processed in a separate pass, so can't affect embedded nodes. [ed: This could be done differently if we used a merge model, rather than a separate process model].
        if (content = context.content(active_property)) && content['@context']
          input['@context'] = case input['@context']
          when nil   then content['@context']
          when Array then input['@context'].concat(content['@context'])
          else            [input['@context'], content['@context']]
          end
        end
        ...
        output_object = {}
        # Process each key and value in element
        expand_object(input, active_property, context, output_object, ordered: ordered)

        # If active_property has an @content definition, process each key and value in that content
        # Processors MUST ensure that content inclusion is not self-referential leading to an unending recursive expansion by noting a cyclic term content error and processing is aborted
        begin
          expand_object(content, active_property, context, output_object, ordered: ordered) if content
        rescue SystemStackError
          raise JsonLdError::CyclicTermContent,
            "The term #{active_property} contains @content referencing itself."
        end
        ...
        if value?(output_object)
          ...
          # If the value of @type is an array with a single element, use that element.
          output_object['@type'] = output_object['@type'].first if output_object['@type'].is_a?(Array) && output_object['@type'].length == 1
  ...
  private
    ...
    # FIXME: check/guard against recursive use of properties with content
    def expand_object(input, active_property, context, output_object, ordered: false)
      ...
      keys.each do |key|
        ...
        if KEYWORDS.include?(expanded_property)
          ...
          # If result has already an expanded property member, an colliding keywords error has been detected and processing is aborted (except for @type, which allows merging).
          ...
          unless expanded_value.nil?
            if output_object[expanded_property]
              # Both should be an array, merge them. This happens only for @type
              output_object[expanded_property].concat(expanded_value)
            else
